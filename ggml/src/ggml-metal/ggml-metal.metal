...
// F8_E4M3 (E4M3 floating point) to float32 conversion
// Follows the IEEE 754 FP8 E4M3 format used by NVIDIA/OCP
static inline float f8_e4m3fn_to_fp32(uint8_t x) {
    ...
    const float val = exp == 0 ? ldexp((float) man, -9) : ldexp(1.0f + (float) man * 0.125f, exp - 7);
...
// F8_E4M3_B128 dequantize: 128 F8_E4M3 values per block with one E8M0 scale
template <typename type4x4>
void dequantize_f8_e4m3_b128(device const block_f8_e4m3_b128 * xb, short il, thread type4x4 & reg) {
    const float d = e8m0_to_fp32(xb->e);
    device const uint8_t * q = xb->qs + 16*il;

    for (int i = 0; i < 4; ++i) {
        reg[i][0] = d * f8_e4m3fn_to_fp32(q[4*i + 0]);
        reg[i][1] = d * f8_e4m3fn_to_fp32(q[4*i + 1]);
        reg[i][2] = d * f8_e4m3fn_to_fp32(q[4*i + 2]);
        reg[i][3] = d * f8_e4m3fn_to_fp32(q[4*i + 3]);
    }
}

template <typename type4>
void dequantize_f8_e4m3_b128_t4(device const block_f8_e4m3_b128 * xb, short il, thread type4 & reg) {
    const float d = e8m0_to_fp32(xb->e);
    const short il4 = il%32;

    device const uint8_t * q = xb->qs + 4*il4;

    reg[0] = d * f8_e4m3fn_to_fp32(q[0]);
    reg[1] = d * f8_e4m3fn_to_fp32(q[1]);
    reg[2] = d * f8_e4m3fn_to_fp32(q[2]);
    reg[3] = d * f8_e4m3fn_to_fp32(q[3]);
}
...
template [[host_name("kernel_mul_mv_ext_f8_e4m3_b128_f32_r1_5")]]  kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<5, block_f8_e4m3_b128,  128, dequantize_f8_e4m3_b128_t4>;
...
template<int NR0, typename args_t>
void kernel_mul_mv_f8_e4m3_b128_f32_impl(
        args_t args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    device const block_f8_e4m3_b128 * x = (device const block_f8_e4m3_b128 *) (src0 + offset0);
    device const float              * y = (device const float              *) (src1 + offset1);

    const int nb   = args.ne00/QK_F8_E4M3_B128;
    const int ns01 = args.nb01/args.nb00;

    const short ti = tiisg;  // 0..31

    float sumf[NR0]={0.f};

    // 32 threads, each handling 4 consecutive values per block
    // 32 × 4 = 128 = QK ✓
    // All threads process all blocks, simd_sum combines
    device const float * yb = y + ti*4;

    for (int ib = 0; ib < nb && ib < ns01; ib += 1) {
        device const float4 * y4 = (device const float4 *) yb;

        const float4 yl = y4[0];

        FOR_UNROLL (short row = 0; row < NR0; row++) {
            device const block_f8_e4m3_b128 & xb = x[row*ns01 + ib];
            device const uint8_t * q = xb.qs + ti*4;

            const float d = e8m0_to_fp32(xb.e);
            sumf[row] += d * (
                f8_e4m3fn_to_fp32(q[0]) * yl[0] +
                f8_e4m3fn_to_fp32(q[1]) * yl[1] +
                f8_e4m3fn_to_fp32(q[2]) * yl[2] +
                f8_e4m3fn_to_fp32(q[3]) * yl[3]);
        }
...
[[host_name("kernel_mul_mv_f8_e4m3_b128_f32")]]
kernel void kernel_mul_mv_f8_e4m3_b128_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    kernel_mul_mv_f8_e4m3_b128_f32_impl<N_R0_F8_E4M3_B128, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg);
}
...
template [[host_name("kernel_get_rows_f8_e4m3_b128")]] kernel get_rows_q_t kernel_get_rows_q<block_f8_e4m3_b128,   8, dequantize_f8_e4m3_b128>;
...
template [[host_name("kernel_mul_mm_f8_e4m3_b128_f32")]] kernel mul_mm_t kernel_mul_mm<half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_f8_e4m3_b128,   8,     dequantize_f8_e4m3_b128,   float,  float4x4,  float, float2x4>;
...
template [[host_name("kernel_mul_mm_f8_e4m3_b128_f16")]] kernel mul_mm_t kernel_mul_mm<half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_f8_e4m3_b128,   8,     dequantize_f8_e4m3_b128,   float,  float4x4,  half, half2x4>;
...
template [[host_name("kernel_mul_mm_id_f8_e4m3_b128_f32")]] kernel mul_mm_id kernel_mul_mm_id<half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_f8_e4m3_b128,   8,     dequantize_f8_e4m3_b128,   float,  float4x4,  float, float2x4>;
...
template [[host_name("kernel_mul_mm_id_f8_e4m3_b128_f16")]] kernel mul_mm_id kernel_mul_mm_id<half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_f8_e4m3_b128,   8,     dequantize_f8_e4m3_b128,   float,  float4x4,  half, half2x4>;
